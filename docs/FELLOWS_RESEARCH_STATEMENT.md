...
Concrete, tractable, current, directly addresses a gap between Arditi 2024 and the 2025–26 follow-ups (Wollschlager, Zhao, Cheng 2604.08524, Maskey 2603.27518, Herring 2605.12290). The headline contribution is the **harmfulness-probe scoring metric applied head-to-head across six techniques**, not the rank-k structure (which is now well-replicated).
...
>
> **What's uncertain in 2026.** Five follow-ups have sharpened the picture in different directions. Wollschlager (ICML 2025) introduced concept cones — refusal lives in a polyhedral subspace, not a line. Zhao (2507.11878) found that ablating the refusal direction on Llama-3-8B / Qwen-7B suppresses verbal refusal but leaves the internal harmfulness judgment detectable. Cheng et al. (2604.08524) and the March 2026 LessWrong replication confirm low-rank refusal on Llama-3.2-3B. Maskey et al. (2603.27518) reframe rank > 1 as a property of *over-refusal* rather than safety-tuning depth. Herring et al. (2605.12290, May 2026) ablate ~0.1% of MLP neurons via contrastive pair search on Llama-3.2-1B/3B. None of these papers score with a Zhao-style residual-stream harmfulness probe; all report keyword- or StrongREJECT-style behavioral metrics.
>
> **What I'd test during the Fellowship.** *Headline:* re-score five published refusal-ablation techniques (Arditi, Wollschlager, COSMIC, Cheng, Maskey) plus Herring CNA on Llama-3.2-1B-Instruct using a Zhao-style residual-stream harmfulness probe. The novel claim: drops in refusal-rate do not imply removal of internal harmfulness representations — the scoring metric itself is the contribution. *Supporting evidence:* the comparison is also the first head-to-head benchmark of these six techniques on the smallest open Llama. *Stretch:* replicate the comparison on Gemma-2-2B-it to test cross-family transfer.
>
> **Differentiation from closest prior work.** Herring et al. (May 2026) recently published the closest comparable work — CNA tested on Llama-3.2-1B with keyword/StrongREJECT scoring. My proposal differs in scoring (residual-stream harmfulness probe, which Herring does not use) and in breadth (head-to-head of six techniques, where Herring compares CNA to Arditi only).
>
> **Why this is tractable.** Harness, probe, and 4/6 techniques (Arditi, Wollschlager cone, COSMIC, Cheng) already shipped in NeuroScope. Remaining: Maskey + Herring CNA + Gemma-2-2B port. Dataset is off-the-shelf (JailbreakBench + Alpaca subset). Compute is consumer-GPU scale; 1B is plausibly browser-loadable, which couples the research finding directly to the engineering thesis.
>
